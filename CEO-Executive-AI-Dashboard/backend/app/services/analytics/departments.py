...
def get_department_performance(db: Session, start_date: str = None, end_date: str = None) -> DepartmentList:
    try:
        ...
        ed = datetime.fromisoformat(end_date) if end_date else datetime.utcnow()
    except ValueError:
        sd = None
        ed = datetime.utcnow()
    ...
    for dept in departments:
        ...
        # Growth (dummy calculation based on a static previous period for demo purposes)
        # In a real app, query previous period exactly like current period
        prev_sd = sd - (ed - sd)
